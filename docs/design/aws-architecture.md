...
graph TB
    ...
    subgraph AWS["AWS"]
        ...
        subgraph AUTH["認証レイヤー"]
            AUTHORIZER["Lambda Authorizer\nrust-aws-lfs-authorizer\n(provided.al2023)"]
        end

        subgraph HANDLER["LFS ハンドラー"]
            LAMBDA["Lambda: LFS Handler\nrust-aws-lfs\n(provided.al2023)"]
        end
        ...
        subgraph CDN["コンテンツ配信"]
            ...
            KEYPAIR["Key Group / Public Key\n(Signed URL 検証)"]
        end

        subgraph IAM["IAM"]
            ROLE_MAIN["Lambda 実行ロール\n(S3: GetObject, PutObject, HeadObject)"]
            ROLE_AUTH["Authorizer 実行ロール\n(基本実行権限のみ)"]
        end
    end
    ...
    %% リクエストフロー
    GIT -->|"HTTPS POST /batch or /verify\n(Authorization: Bearer <github-token>)"| ROUTE
    ROUTE -->|"認可チェック"| AUTHORIZER
    AUTHORIZER -->|"GET /repos/{owner}/{repo}\n(トークン検証)"| GITHUB_API
    GITHUB_API -->|"permissions.pull 確認"| AUTHORIZER
    AUTHORIZER -->|"isAuthorized: true/false"| ROUTE
    ROUTE -->|"認可済みリクエスト"| LAMBDA

    %% Lambda ↔ S3
    LAMBDA -->|"HeadObject / PutObject (Presigned URL 生成)"| S3
    LAMBDA -->|"Signed URL 生成"| CF

    %% アップロード: クライアントが直接 S3 へ
    GIT -.->|"PUT (Presigned URL 経由)\n直接アップロード"| S3

    %% ダウンロード: クライアントが CloudFront 経由
    GIT -.->|"GET (CloudFront Signed URL 経由)\n直接ダウンロード"| CF
...
sequenceDiagram
    ...
    alt オブジェクトが存在する
        S3-->>LFS: 200 OK
        LFS->>LFS: CloudFront Signed URL 生成\n(RSA 署名 + 有効期限)
...
graph LR
    subgraph "認証・認可"
        A["GitHub Token\n(HTTP Basic Auth)"] -->|"Lambda Authorizer で検証"| B["GitHub API\n/repos/{owner}/{repo}"]
