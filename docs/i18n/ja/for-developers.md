...
> **注意:** この手順は Terraform では自動化できません。AWS コンソールから手動で実施してください。

**前提条件：**
- Terraform のデプロイが完了していること（`main.tf` には定額プランに必要な `Managed-CachingOptimized` が設定済み）
