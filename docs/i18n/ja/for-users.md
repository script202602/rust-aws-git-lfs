...
> **⚠️ リージョンを合わせること：** バケットは CloudFormation スタックと**同じリージョン**に作成する必要があります。
> リージョンが異なると `PermanentRedirect` エラーが発生し、Lambda 関数の作成に失敗します。
...
3. パラメータを入力：
   ...
   **任意（デフォルトのままでも動作します）：**
   ...
   | AllowedGithubUsers | *(空)* | 認証を許可する GitHub ユーザー名（カンマ区切り）。例：`alice,bob`。空の場合はリポジトリへの pull 権限を持つ任意のユーザーを許可する。 |
...
スタック作成時に以下のエラーが発生した場合：

```
Resource of type 'AWS::Logs::LogGroup' with identifier '...' already exists.
```

以前のスタック削除でロググループが残っています。削除してから再試行してください：

1. AWS コンソール → **CloudWatch** → **ロググループ**（左サイドバー）
2. 検索欄に `/aws/lambda/<スタック名>` を入力（例：`/aws/lambda/git-lfs`）
3. 表示された `-rsa-key-gen`・`-authorizer`・`-main` のロググループをすべて選択
4. **アクション** → **ロググループの削除** → 確認

削除後、スタックの作成を再試行してください。
